...
class BinaryOps(Enum):
  """A + A -> A (elementwise)"""
  ADD = auto(); SUB = auto(); MUL = auto(); IDIV = auto(); MAX = auto(); MOD = auto(); CMPLT = auto(); CMPNE = auto(); XOR = auto(); DIV = auto(), # removing DIV = auto() can't find the gpu # noqa: E702
...
python_alu = {
  ...
  UnaryOps.RECIP: lambda x: 1 / x if x != 0 else (-math.inf if x <= -0 else math.inf),  #RECIP
